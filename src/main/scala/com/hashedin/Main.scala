...
import org.apache.spark.sql.functions.{col, count, max, struct, sum}


object Main {
  def main(args: Array[String]): Unit = {

    val csvFileInputPath = "/home/hasher/Downloads/imdb_movies.tsv"
    ...
    val englishLanguageRDD = csvRDD.filter(_.contains("English")).map(line => line.split("\t")(3).toInt).countByValue()
    println("englishLanguageRDD")


    englishLanguageRDD.foreach(println)

    println("End englishLanguageRDD")
    ...
    val awardWinningTitlesRDD = csvRDD.filter(_ != csvHeaderRDD).map(line => {
      val col = line.split("\t")
      (col(2), col(0),col(3), col(15).toLong)
    }).groupBy(x => x._3).mapValues(_.maxBy(_._4)).values
    println("AwardWiningTitle")
    awardWinningTitlesRDD.foreach(println)
    println("EndAwardWiningTitle")
    ...
    println("Json EndAwardWiningTitle")
